...
(defn collection
  [name]
  (str "com.etzhayyim.uchiwake." name))

(def cell-specs {
  :ingest {:legacy-cell "cell-uchiwake-ingest"
     :phase :event
     :murakumo-node "reuben"
     :collections [(collection "ingest")]
     :required-gates common-gates
     :trigger "manifest cell ingest"
     :ceiling "Manifest-driven migration scaffold; explicit execution stays in runtime methods"}
  :analyze {:legacy-cell "cell-uchiwake-analyze"
     :phase :event
     :murakumo-node "reuben"
     :collections [(collection "analyze")]
     :required-gates common-gates
     :trigger "manifest cell analyze"
     :ceiling "Manifest-driven migration scaffold; explicit execution stays in runtime methods"}
  :crosscheck {:legacy-cell "cell-uchiwake-crosscheck"
     :phase :event
     :murakumo-node "reuben"
     :collections [(collection "crosscheck")]
     :required-gates common-gates
     :trigger "manifest cell crosscheck"
     :ceiling "Manifest-driven migration scaffold; explicit execution stays in runtime methods"}
})
